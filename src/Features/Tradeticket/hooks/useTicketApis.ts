...
import { useGetBondListQuery, useGetCptysQuery, useGetCurrencyListQuery, useGetNewTicketIdQuery } from '../../../services'
import { BondListResponse, CptyResponse, CurrencyResponse, NewTicketIdResponse } from '../../../types'
...
export const useTicketApis = (): ReturnProps => {
    const { data: ticketId, isError: ticketIdError, isSuccess: isTicketIdSuccesss } = useGetNewTicketIdQuery();
    ...
    useEffect(() => {
        if (isTicketIdSuccesss && isBondListSuccess && isCptySuccess && isCcySuccess) {
            setLoading(false);
        }
